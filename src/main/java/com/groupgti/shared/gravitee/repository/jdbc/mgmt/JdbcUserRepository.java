...
import com.groupgti.shared.gravitee.repository.jdbc.mgmt.JdbcHelper.ChildAdder;
import com.groupgti.shared.gravitee.repository.jdbc.mgmt.JdbcHelper.CollatingRowMapper;
...
public class JdbcUserRepository implements UserRepository {
    ...
    public Optional<User> findByUsername(String username) throws TechnicalException {
        ...
        try {
            ...
            jdbcTemplate.query("select * from User u left join UserRole ur on u.Username = ur.Username where u.Username = ?"
                    , rowMapper
                    , username
            );
    ...
    public Set<User> findByUsernames(List<String> usernames) throws TechnicalException {
        ...
        try {
            CollatingRowMapper<User> rowMapper = new CollatingRowMapper<>(ORM.getRowMapper(), CHILD_ADDER, "Username");
            jdbcTemplate.query("select * from User u left join UserRole ur on u.Username = ur.Username where u.Username in ( " 
    ...
    public Optional<User> findByEmail(String email) throws TechnicalException {
        ...
        try {
            CollatingRowMapper<User> rowMapper = new CollatingRowMapper<>(ORM.getRowMapper(), CHILD_ADDER, "Username");
            jdbcTemplate.query("select * from User u left join UserRole ur on u.Username = ur.Username where u.Email = ?"
                    , rowMapper
                    , email
            );
            List<User> users = rowMapper.getRows();
    ...
    public Set<User> findAll() throws TechnicalException {
        ...
        try {
            CollatingRowMapper<User> rowMapper = new CollatingRowMapper<>(ORM.getRowMapper(), CHILD_ADDER, "Username");
            jdbcTemplate.query("select * from User u left join UserRole ur on u.Username = ur.Username"
                    , rowMapper
            );
            List<User> users = rowMapper.getRows();
            return new HashSet<>(users);
    ...
    public Set<User> findByTeam(String teamName) throws TechnicalException {
        ...
        try {
            CollatingRowMapper<User> rowMapper = new CollatingRowMapper<>(ORM.getRowMapper(), CHILD_ADDER, "Username");
            jdbcTemplate.query("select * from User u "
